...
void CloudProjection::get_matches_by_direction(Eigen::Matrix4f transform, int direction_index) {
    ...
    for (size_t i = 0; i < p_rotated_old_pcl->points.size(); ++i) {
        if (p_rotated_old_pcl->points[i].x < x_min)
            x_min = p_rotated_old_pcl->points[i].x;
        if (p_rotated_old_pcl->points[i].y < y_min)
            y_min = p_rotated_old_pcl->points[i].y;
        if (p_rotated_old_pcl->points[i].z < z_min)
            z_min = p_rotated_old_pcl->points[i].z;
        if (p_rotated_old_pcl->points[i].x > x_max)
            x_max = p_rotated_old_pcl->points[i].x;
        if (p_rotated_old_pcl->points[i].y > y_max)
            y_max = p_rotated_old_pcl->points[i].y;
        if (p_rotated_old_pcl->points[i].z > z_max)
            z_max = p_rotated_old_pcl->points[i].z;
    }
    for (size_t i = 0; i < p_rotated_new_pcl->points.size(); ++i) {
        if (p_rotated_new_pcl->points[i].x < x_min)
            x_min = p_rotated_new_pcl->points[i].x;
        if (p_rotated_new_pcl->points[i].y < y_min)
            y_min = p_rotated_new_pcl->points[i].y;
        if (p_rotated_new_pcl->points[i].z < z_min)
            z_min = p_rotated_new_pcl->points[i].z;
        if (p_rotated_new_pcl->points[i].x > x_max)
            x_max = p_rotated_new_pcl->points[i].x;
        if (p_rotated_new_pcl->points[i].y > y_max)
            y_max = p_rotated_new_pcl->points[i].y;
        if (p_rotated_new_pcl->points[i].z > z_max)
            z_max = p_rotated_new_pcl->points[i].z;
    }
